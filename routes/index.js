...
router.get('/usermanagement', function (req, res) {
    ...
    var query = "SELECT A.Name,A.Email,A.Gender,A.Date_of_Birth,A.Age,A.Address,A.Contact,A.Blood_Group,A.Marital_Status,A.Join_Date,A.Salary,A.Username,B.Password,B.Usertype FROM user_information A INNER JOIN user_access B ON A.Email=B.Email;";
    db.getData(query, null, function (rows) {
        var data = {
...
router.get("/batch/expiry", (req, res) => {
    ...
    let ans = "";
    var query = "SELECT b.*, m.Medicine_Name, s.Supplier_Name FROM batch b INNER JOIN medicine_information m on b.Medicine_ID = m.ID INNER JOIN supplier s on b.Supplier_ID = s.ID WHERE b.expire_date BETWEEN CURDATE() and DATE_ADD(CURDATE(), INTERVAL 30 DAY)";
    ...
    connection.query(query, (err, results, fields) => {
        if (err) {
            ...
            // console.log(data);
            res.render('expiry_index', data);
        }
    })

})

router.get("/sendReminder/:id", (req, res) => {
    // staff checking
    // check_staff(req, res);
    var connection = mysql.createConnection({
        host: 'localhost',
    ...
    const query = `SELECT b.Batch_ID,b.Quantity,b.Cost_Price,b.Sell_Price,b.Expire_Date,b.Production_Date, m.Medicine_Name, s.email,s.Supplier_Name FROM batch b INNER JOIN medicine_information m on b.Medicine_ID = m.ID INNER JOIN supplier s on b.Supplier_ID = s.ID WHERE b.id= ${req.params.id}`;
    connection.query(query, (err, results, fields) => {
        if (err) {
            console.log(err);
        } else {
            let info = "";
            info += `
                The Following Medicine is expiring in One Month : <br>
                Batch Id: ${results[0].Batch_ID},<br>
                Medicine Name : ${results[0].Medicine_Name},<br>
                Quantity : ${results[0].Quantity},<br>
                Cost Price : ₹${results[0].Cost_Price},<br>
                Sale Price : ₹${results[0].Sell_Price},<br>
                Production Date : ${results[0].Production_Date},<br>
                Expiration Date: ${results[0].Expire_Date}<br>
                Kindly Discard It.<br>
                Thank You!
            `
            console.log(results.length);
            sendMail(results[0].email, "PMS Medicine Expiry", info);
            res.redirect("back");
        }
    })
